...
class DataPreprocessService:
    ...
    async def preprocess_article(self, article: New) -> List[PreprocessEventSchema]:
        events_dicts = await self.get_events_dicts(article.body)
        preproc_events = []
        for event_dict in events_dicts:
            try: 
                preproc_event = PreprocessEventSchema(title=article.headline, **event_dict)
